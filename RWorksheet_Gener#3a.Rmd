...
its output?
```{r row_3_and_4 <- weather_data[3:4, ]}
```
...
```
c. 
Name the rows as lowercase letters and columns as uppercase letters starting from
...
dimnames(my_array) <- list("1st-Dimensional Array" = rownames(my_array),
                             "2nd-Dimensional Array" = colnames(my_array),
